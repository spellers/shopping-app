...
const out = await page.evaluate(async () => {
  ...
  // try candidate siteIds
  for (const sid of ["ASDA", "Sites-ASDA", "asda-uk", "UK"]) {
    const r = await call("POST", `/baskets?siteId=${sid}`, { customerName: { firstName: "Test", lastName: "Recon" }, currency: "GBP", listName: "Groceries", listType: "Groceries", type: "Groceries", language: "en-GB" });
    res["create_" + sid] = r;
    if (r.status === 200) {
      const bid = JSON.parse(r.body).basketId;
      res.basketId = bid; res.siteId = sid;
      res.get = await call("GET", `/baskets/${bid}?siteId=${sid}`);
      let cin = null;
      const a = document.querySelector('a[href*="/p/"]');
      if (a) { const m = a.getAttribute("href").match(/\/p\/([0-9]+)/); cin = m && m[1]; }
      res.cin = cin;
      if (cin) {
        res.add = await call("PUT", `/baskets/${bid}/items/${cin}?siteId=${sid}`, [
          { productCode: cin, quantity: 1, quantityAvailable: null, quantityBackOrdered: null, itemIndex: 1, lineItemNumber: 1, type: "Product" }
        ]);
        res.get2 = await call("GET", `/baskets/${bid}?siteId=${sid}`);
      }
      break;
    }
  }
...
await writeFile("/tmp/asda_recon19.json", JSON.stringify(out, null, 2));
