...
const out = await page.evaluate(async () => {
  ...
  if (create.status === 400 && cb.basketIds) {
    for (const id of cb.basketIds.split(",")) await call("DELETE", `/baskets/${id}?${S}`, null);
  ...
  res.cin = cin;
  res.post = await call("POST", `/baskets/${bid}/items?${S}`, [{ productId: cin, quantity: 1, c_allowSubstitutes: true }]);
  const pb = JSON.parse(res.post.body || "{}");
  const items1 = (pb.shipments || []).flatMap(s => s.productItems || []);
  res.items = items1.map(i => ({ code: i.productCode, name: i.productName, qty: i.quantity, price: i.priceValue, itemId: i.itemId }));
  res.totals = { productTotal: pb.productTotal, orderTotal: pb.orderTotal };
  if (items1.length) {
    const it = items1[0];
    res.patch = (await call("PATCH", `/baskets/${bid}/items/${it.itemId}?${S}`, { productId: it.productCode, quantity: 2, c_allowSubstitutes: true })).status;
...
await writeFile("/tmp/asda_recon29.json", JSON.stringify(out, null, 2));
